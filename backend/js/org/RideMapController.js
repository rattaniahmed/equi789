app.controller('RideMapController', function ($scope, storageService, firebaseService, $firebaseArray, $routeParams, $rootScope) {
    ...
    $scope.GOrganisation = JSON.parse(localStorage.getItem('adminObject'));
    ...
    function addMarker(rideObj, location) {
        ...
        //var markerCluster = new MarkerClusterer($scope.map, marker,
    ...
    function addMarkerForEH1(rideObj) {
        var contentString = '<div id="content" style="margin-left:20px;margin-top:20px;">' + '<div center style="text-align:center;object-fit:cover;"><img style="width:150px;height:150px" src=' + $scope.GOrganisation.Url + ' /></div>' +
            '<div id="siteNotice">' +
            '</div>' +
            '<div style="text-align: center;"><h4 id="firstHeading" class="firstHeading">' + rideObj.location + '</h4></div>' +
            '<div id="bodyContent">' +
    ...
    $scope.InitEH1 = function (locations) {
        ...
        if ($rootScope.isDataLoaded) {
            ...
            $scope.InitMapForEH1(centerCord, locations);
            UnLoadingState();
        }
    }
